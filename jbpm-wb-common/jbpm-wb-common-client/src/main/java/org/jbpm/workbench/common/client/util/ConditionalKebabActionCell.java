...
public class ConditionalKebabActionCell<T extends GenericSummary> extends AbstractCell<T> {
    ...
    public void render(final Cell.Context context,
        ...
        if (availableActions.size() > 1) {
            ...
            final List<ConditionalAction<T>> actionMenus = availableActions.stream().filter(action -> action.isNavigational() == false).collect(Collectors.toList());
            ...
            final List<ConditionalAction<T>> navigationMenus = availableActions.stream().filter(action -> action.isNavigational()).collect(Collectors.toList());
